...
async function captureConsoleErrors(page) {
  ...
  page.on('console', (message) => {
    if (message.type() === 'error' || /hydration|NG0295[25]/i.test(message.text())) errors.push(message.text());
  });
  return errors;
}
...
test('article direct load and client navigation expose one valid article schema', async ({ page }) => {

...
test("validates a publishable post and deterministic date/slug ordering", async () => {
  ...
  assert.ok(
    comparePosts(
      { date: "2025-01-01", slug: "zeta" },
      { date: "2025-01-01", slug: "alpha" },
    ) > 0,
  );
});

test("validates an author avatar and detects duplicate case-insensitive slugs", async () => {
  const { contentDir, imagesDir } = await fixture();
  await fs.mkdir(path.join(imagesDir, "authors"));
  await fs.writeFile(path.join(imagesDir, "authors", "avatar.webp"), "avatar");
  await fs.mkdir(contentDir);
  const valid =
    "---\ntitle: Alpha\ndescription: d\ndate: 2025-01-01\ncategories: [A]\nauthor:\n  name: Author\n  avatar: authors/avatar.webp\n---\nbody";
  await fs.writeFile(path.join(contentDir, "alpha.md"), valid);
  await fs.writeFile(path.join(contentDir, "alpha.MD"), valid);
...
test("reports malformed fields, invalid slug, missing image, and traversal", async () => {
  ...
  await fs.mkdir(contentDir);
  await fs.writeFile(
    path.join(contentDir, "Not Safe.md"),
    "---\ntitle: 4\ndescription: \ndate: 2025-02-30\ncategories: nope\nimage: ../secret.webp\n---\nbody",
  );
...
test("does not publish an explicit draft", async () => {
  ...
  assert.deepEqual(validateContentDirectory(contentDir, imagesDir), {
    errors: [],
    posts: [],
  });
});

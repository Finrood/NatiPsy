...
describe('SeoService', () => {
  ...
  it('should update title and meta tags', () => {
    ...
    expect(metaService.getTag('property="og:url"')?.content).toBe(
      'https://psicologanataliaferreira.com/test',
    );
  });
  ...
  it('should generate stable canonical URLs without query strings or fragments', () => {
    ...
    Object.defineProperty(router, 'url', {
      value: '/blog?fbclid=IwARabc123&utm_source=instagram',
      configurable: true,
    });
    ...
    Object.defineProperty(router, 'url', {
      value: '/blog/carreira-mulheres-negras-fadiga-racial#comentarios',
      configurable: true,
    });
    service.updateMetaTags({ title: 'Post', description: 'd' });
    expect(document.querySelector('link[rel="canonical"]')?.getAttribute('href')).toBe(
      `${SITE_URL}/blog/carreira-mulheres-negras-fadiga-racial`,
    );
  ...
  it('should manage article:tag Open Graph entries without leaking stale ones', () => {
    ...
    const tags = () =>
      Array.from(document.querySelectorAll<HTMLMetaElement>('meta[property="article:tag"]'));

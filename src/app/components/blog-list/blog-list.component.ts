...
function parseQuery(params: Params): BlogQueryState {
  ...
  const page = typeof pageValue === 'string' && /^[1-9]\d*$/.test(pageValue) && Number.isSafeInteger(Number(pageValue))
    ? Number(pageValue)
    : 1;
  ...
  return {
    page,
    category: typeof firstQueryValue(params['category']) === 'string' ? firstQueryValue(params['category']) as string : '',
...
function sameQuery(a: BlogQueryState, b: BlogQueryState): boolean {
  return a.page === b.page && a.category === b.category && a.sortBy === b.sortBy && a.sortDirection === b.sortDirection;
}

function serializeQuery(state: BlogQueryState): Record<string, string | number | null> {
  return {
...
export class BlogListComponent implements OnInit {
  ...
  private readonly queryState$ = this.route.queryParams.pipe(
    map(parseQuery),
  ...
  private readonly filterState$ = this.queryState$.pipe(
    ...
    distinctUntilChanged((a, b) => a.category === b.category && a.sortBy === b.sortBy && a.sortDirection === b.sortDirection),
  );
  ...
  readonly viewModel = toSignal(
    combineLatest({ query: this.queryState$, content: this.contentState$ }).pipe(
      map(({ query, content }): BlogListViewModel => {

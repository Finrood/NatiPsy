...
try {
  ...
  await writeFile(join(contentDir, 'draft.md'), post('draft', '2026-01-04', 'draft: true\n'));
  await writeFile(join(contentDir, 'unpublished.md'), post('unpublished', '2026-01-05', 'published: false\n'));
  await writeFile(join(contentDir, 'invalid.md'), '---\ndescription: missing title\n---\nignored\n');
  for (let index = 0; index < 21; index += 1) {
    await writeFile(join(contentDir, `post-${index}.md`), post(`post-${index}`, `2025-02-${String(index + 1).padStart(2, '0')}`));
  }

  runGenerator();
  const firstFeed = await readFile(feedPath, 'utf8');
  const firstIndex = JSON.parse(await readFile(outputIndex, 'utf8'));
  assert.equal(firstIndex.some(item => item.slug === 'draft' || item.slug === 'unpublished'), false);
  assert.equal(firstIndex.some(item => item.slug === 'invalid'), false);
  assert.equal((firstFeed.match(/<item>/g) ?? []).length, 20);
  assert.match(firstFeed, /Descrição com &lt;tag&gt; &amp; acento/);
  assert.match(firstFeed, /<title>newest<\/title>/);
  assert.doesNotMatch(firstFeed, /<title>post-0<\/title>/);

  runGenerator();
  assert.equal(await readFile(feedPath, 'utf8'), firstFeed, 'feed generation must be deterministic');

  await rm(join(contentDir, 'newest.md'));
  runGenerator();
  const changedFeed = await readFile(feedPath, 'utf8');
  assert.doesNotMatch(changedFeed, /<title>newest<\/title>/, 'removing a post must remove it from the feed');
} finally {

...
try {
  ...
  assert.throws(
    () =>
      execFileSync(process.execPath, ['scripts/report-bundle-size.mjs'], {
        env: {
          ...process.env,
  ...
  await writeFile(
    join(fixtureRoot, 'browser-only-baseline.json'),
    JSON.stringify({ initialBytes: 500000 }),
  );
  await writeFile(
    join(fixtureRoot, 'browser-only-stats.json'),
    JSON.stringify({
      outputs: {
  ...
  assert.deepEqual(
    report.lazyFiles.map((file) => file.asset),
    ['lazy.js'],
  );
} finally {

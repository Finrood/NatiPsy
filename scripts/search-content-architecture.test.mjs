...
const [routes, sitemap, routeConfig, pageTemplate, pageSource, footer, generator, staticPages, trustTemplate] = await Promise.all([
  read('src/routes.txt'),
...
assert.match(footer, /routerLink="\/contato-e-privacidade"/, 'footer must link to trust and contact information');
assert.match(pageTemplate, /Natalia Ferreira dos Santos · Psicóloga · CRP 12\/19892/, 'service pages must show the full professional identity');
assert.match(pageTemplate, /não é um canal de emergência/, 'the service CTA must state the WhatsApp crisis boundary');
assert.match(pageTemplate, /SAMU \(192\).*CVV \(188\)/s, 'the service CTA must provide official urgent directions');
assert.match(trustTemplate, /Privacidade e escopo/, 'the linked trust page must explain initial-contact privacy and scope');
assert.match(trustTemplate, /Política editorial/, 'the linked trust page must state the editorial policy');
...
for (const route of ['/terapia-online', '/orientacao-profissional', '/contato-e-privacidade']) {
  ...
  assert.match(page.reviewedAt, /^\d{4}-\d{2}-\d{2}$/, `${route} must have an explicit reviewedAt date`);
  assert.match(
    sitemap,
    new RegExp(`<loc>https://psicologanataliaferreira\\.com${route}<\\/loc>\\s*<lastmod>${page.reviewedAt}<\\/lastmod>`),
    `${route} sitemap freshness must come from its content review date`,
  );
}
assert.match(generator, /static-pages\.json/, 'sitemap generation must consume the content registry');

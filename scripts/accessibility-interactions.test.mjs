...
const [topMenu, blogList, blogPost] = await Promise.all([
...
for (const [name, template] of [['top-menu', topMenu], ['blog-list', blogList], ['blog-post', blogPost]]) {
  ...
  const scaleClasses = template.match(/(?:motion-safe:)?(?:group-)?hover:scale(?:-[^\s"]+)?/g) ?? [];
  assert.ok(scaleClasses.every((className) => className.startsWith('motion-safe:')),
    name + ' scale-on-hover interactions must respect prefers-reduced-motion');
}

assert.match(topMenu, /isMenuOpen\(\) \? 'text-white focus:ring-white' : 'text-primary-blue focus:ring-primary-blue'/,
  'mobile menu toggle must keep its focus indicator visible on both backgrounds');
assert.match(topMenu, /\[attr\.aria-expanded\]="isMenuOpen\(\)\.toString\(\)"/,
...
assert.match(blogList, /<div class="block h-48 bg-gray-200 overflow-hidden" aria-hidden="true">[\s\S]*?\[ngSrc\]/,
  'blog card media must not create a second keyboard stop');
assert.doesNotMatch(blogList, /Leia mais|Read more/, 'blog cards must not repeat their title link with a read-more link');
assert.equal((blogList.match(/\[routerLink\]="\['\/blog', post\.slug\]"/g) ?? []).length, 1,
...
assert.ok(buttons.every((button) => /\btype="button"/.test(button)),

...
const styles = readFileSync(
  new URL('../src/app/components/blog-post/blog-post.component.css', import.meta.url),
  'utf8',
);
...
assert.match(template, /<nav\s+aria-label="Neste artigo"/);
assert.match(template, /post\.headings\.length >= 2/);
assert.match(template, /\[routerLink\]="\['\/blog', post\.slug\]"/);
assert.match(template, /\[fragment\]="heading\.id"/);
assert.match(template, /<h1\s+tabindex="-1"/);
